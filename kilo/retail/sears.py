	...
	<div class='cell'>
		<div id='unit'>
			...
			<div>
				<div class='price'>
					<a href='%(cjpn)s' id='%(uniqid)s' target="_blank">
						<button type="button" data-toggle="tooltip" title="Buy from Sears.com"
...
def get_cells( f, sears_laptop_objects):
	...
	for x in range( len( sears_laptop_objects)):
		...
		url = """http://www.jdoqocy.com/click-%(pid)s-11515703?url=http://www.Sears.com/s/p-%(partnumber)s"""
		url = url % { 'pid': '7904552', 'store':get_store_name(i['store']), 'partnumber':i['partnumber']}

		i['cjpn'] = url
		i['link'] = i['img']
		i['uniqid'] = i['partnumber']
			
		if store_lookup.has_key( i['store'] ):
			i['store'] = store_lookup[i['store']]
